<script lang="ts">
	import { AppUser, type Post } from "$lib/interfaces";
	import { appService } from "$lib/app-service";
	import { onMount } from "svelte";
  import UserPopup from "$lib/component-user-popup.svelte";
  
  export let post: Post;
  let currentUser: AppUser | undefined = appService.currentUser;
  let postUser: AppUser = new AppUser(post.authorId, "test@example.com", post.authorDisplayName, post.authorDisplayName, post.authorPhotoUrl);
...
<div class="container" on:mouseleave={hideUserPopup}>
  {#if post}
    <div class="frame">
      <div class="profile">
        {#if userPopupVisible && currentUser}
          <UserPopup onEnter={showUserPopup} onLeave={hideUserPopup}
           localUser={currentUser} user={postUser} 
           size="large" userFollowing={userFollowing}
           {followingChanged}/>
        {/if}
        <img
          alt="user profile"
      ...
      <div class="bylines">
        <div class="byline_left">
          <div class="byline_author">
            <a href={"/" + post.authorDisplayName}>{post.authorDisplayName}</a>
          </div>
          <div class="byline_details">
            {new Date(post.createdAt).toDateString()} · 2 min read · Listen
          </div>
        </div>
